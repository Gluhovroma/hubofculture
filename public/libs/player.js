...
(function() {
  ...
  this.AudioPlayer = (function() {
    ...
    AudioPlayer.prototype.biteState = function() {
      _ref=this.ui;
      state=this.state;
      state==3?_ref.goToSong(0):clearInterval(this.intId);
    }
    ...
    AudioPlayer.prototype.isLoading = function() {
      if (!this.state && this.isEmpty()) {
        $('#audio-player').trigger('playerStopLoading');
        return false;
      }
      $('#audio-player').trigger('playerStopLoading2');
    ...
    AudioPlayer.prototype.isEnded = function() {
      // $('#audio-player').trigger('playerEnded');
  ...
  this.AudioPlayerUI = (function() {
    ...
    AudioPlayerUI.prototype.setEl = function(el) {
      ...
      this.$backButton = this.$el.find(".ion-ios-rewind");
      this.$nextButton = this.$el.find(".ion-ios-fastforward");
    ...
    AudioPlayerUI.prototype.togglePlayPause = function() {
      if (this.audioPlayer.isPlaying()) {
        if (audioPlayer.songs[0]['srcs'][0]['src'] == "http://94.250.252.115:8000/live") {
          $('#audio-player').trigger('radioplaypause');
          this.audioPlayer.pause();
          audioPlayer.songs = [];
          console.log(this.audioPlayer);
          
          // $("#audio-player audio").attr("src", sourceUrl);
        ...
        if (audioPlayer.songs.length == 0) {
                var songs = [{
                    ...
                    srcs: [
                      {
                        src: "http://94.250.252.115:8000/live",
                        type: "audio/mp3"
                      }
                    ]
                  }];
                audioPlayer.songs = songs;
                audioPlayer.goToSong(0)                
     ...
     AudioPlayerUI.prototype.togglePlay = function() {      
        return this.audioPlayer.play();
      
    };

     AudioPlayerUI.prototype.togglePause = function() {      
        return this.audioPlayer.pause();      
    };
    ...
    AudioPlayerUI.prototype.previousSong = function() {
    	if (this.songs.length >1 ) {
    		$('#audio-player').trigger('previousSong');
    ...
    AudioPlayerUI.prototype.AudioPlayerUpdateState = function() {
      ...
      if (this.audioPlayer.isPlaying()) {
        this.$button.removeClass("ion-ios-play").addClass("ion-ios-pause");
      } else {
        this.$button.removeClass("ion-ios-pause").addClass("ion-ios-play");
      }
    ...
    AudioPlayerUI.prototype._unbindEvents = function() {
      ...
      AudioPlayerUI.prototype.radioPlay = function() {      
        var el = [{
        image: "images/sunhawk-small-2@2x.jpg",
        name: "",
        srcs: [
     ...
     AudioPlayerUI.prototype.PlayerPlay = function() {      
           this.togglePlay();      
       
    };

    AudioPlayerUI.prototype.PlayerMyPause = function() {      
           this.togglePause();      
       
    };
    AudioPlayerUI.prototype.playspecificSong = function(song) {      
    ...
    AudioPlayerUI.prototype.songPlay = function(songs) {         
      this.songs = songs;
      this.goToSong(0);
      this.togglePlay();       
    };
